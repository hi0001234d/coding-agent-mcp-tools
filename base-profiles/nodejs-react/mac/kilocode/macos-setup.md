...
#  Node.js + React/Next.js Profile — macOS
#
...
if ! command -v go &> /dev/null; then
    ...
    if command -v brew &> /dev/null; then
        ...
        rm /tmp/go.tar.gz
        log_info "Go $GO_VERSION installed (arm64)"
    fi
...
if ! command -v pip3 &> /dev/null; then
    log_info "pip3 not found — installing Python via Homebrew..."
    if command -v brew &> /dev/null; then
        brew install python3
    else
        log_error "pip3 not found and Homebrew not available. Install Python first."
        log_warn "See: https://www.python.org/downloads/macos/"
        exit 1
    fi
fi
